...
lector = csv.DictReader(csv_data.splitlines())
salarios = [int(fila["salario"]) for fila in lector]
media = sum(salarios) / len(salarios)
print(f"Salario medio: {media:.2f}")

# Ejercicio 6: Añadir fila a CSV existente
# Abre data/empleados.csv en modo append ('a', newline='') y añade un nuevo empleado.
with open(DATA_DIR / "empleados.csv", "a", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=["id", "nombre", "departamento", "salario"])
    w.writerow({"id": 3, "nombre": "Sofia", "departamento": "IT", "salario": 42000})
...
it_empleados = [e for e in empleados if e["departamento"] == "IT"]
with open(DATA_DIR / "it_empleados.csv", "w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=["id", "nombre", "departamento", "salario"])
    w.writeheader()
    w.writerows(it_empleados)
print("Archivo it_empleados.csv creado")
